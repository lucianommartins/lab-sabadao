# -*- coding: utf-8 -*-
# Vendored from T-Eval (github.com/open-compass/T-Eval, review_evaluator.py) for leaderboard-faithful
...
import json
import json as _mmless_json
def load(path):
    with open(path, encoding='utf-8') as _f:
        return _mmless_json.load(_f)

from .template import parse_string
from .schema import ResponseDataSample
import ast
import numpy as np
try:  # gbench vendoring delta: lazy so `import eval_suites` works without sentence-transformers;
    from sentence_transformers import SentenceTransformer, util  # t_eval hard-errors at run time if absent.
